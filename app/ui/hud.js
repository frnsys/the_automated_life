    ...
    <div className='hud'>
      ...
      <div className='hud-progress'>
        ...
        <div className='stat-group' data-tip={t('retirement_savings_remaining', {amount: numeral(Math.max(0, config.retirementSavingsMin - props.player.cash)).format('0,0.0a')})}>
          ...
          <div className='bar' style={{border: props.player.cash < 0 ? '1px solid red' : ''}}><div className='bar-fill' style={{width: `${props.player.cash < 0 ? Math.min(1, props.player.cash/config.gameOverBalance)*100 : Math.min(1, props.player.cash/config.retirementSavingsMin)*100}%`, background: `${props.player.cash < 0 ? '#ffacac' : ''}`}} />{props.player.cash < 0 ? <div className='stat-debt'>{numeral(props.player.cash).format('$0,0.0a')}</div> : ''}</div>
        </div>
      </div>
    </div>
  );
